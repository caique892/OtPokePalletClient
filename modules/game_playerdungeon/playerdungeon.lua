...
selectedCategory = "Iniciante" -- Mostrar a categoria selecionada
...
function init()
    ...
    dungeonButton = modules.client_topmenu.addRightGameToggleButton('DungeonButton', tr('Dungeon'), 'dungeon.png', exibir)
...
function onReceiveDungeonDifficulty(buffer)
    ...
    label.onClick = function()
        for i = 1, #lista do
            ...
            label:setColor("white")
            lista2[numeration]:setColor("white")
...
function onReceiveDungeonMissions(buffer)
    ...
    if string.find(title, "Toca dos Rattata's") then
        label:setColor("#0083FF")
        image:setImageSource("dungeons/rattata.png")
    end
    ...
    favorit:setMarginLeft(354)
    favorit:setMarginTop(57)
    ...
    chest:setMarginLeft(377)
    chest:setMarginTop(57)
    ...
    ranking:setMarginLeft(399)
    ranking:setMarginTop(57)

    local players = g_ui.createWidget("Label", label)
    players:setColor("white")
